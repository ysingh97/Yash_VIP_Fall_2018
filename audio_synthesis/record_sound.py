...
class AudioRecorder():
    def __init__(self):
        ...
        self.WAVE_OUTPUT_FILENAME = os.getcwd() + "/sounds/recordings/voice_recordings" + str(len(files)) + ".wav"
        self.pyAudio = pyaudio.PyAudio()
        self.instruments = ['drums', 'cello', 'drum', 'cellos']
    ...
    def record(self):
        ...
        stream = self.pyAudio.open(format = self.FORMAT, channels = self.CHANNELS, rate = self.RATE, input = True, output = True, frames_per_buffer = self.CHUNK, input_device_index = 11)
        frames = array('h')
        ...
        instrument = ""
        if(words[len(words)-1] in self.instruments):
            instrument= words[len(words) - 1]
